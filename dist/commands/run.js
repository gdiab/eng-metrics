...
function buildSearchQuery(org, repos, startIso, endIso) {
    ...
    if (repos.mode === 'allowlist' && repos.allowlist.length > 0) {
        ...
        if (repoQueries.length === 0) {
            throw new Error(`Cannot search without org: repos in allowlist must be in "owner/repo" format (e.g., "gdiab/my-repo"). ` +
                `Found: ${repos.allowlist.join(', ')}`);
        }
        return `${repoQueries.join(' ')} is:pr ${timeRange}`;
    }
    // No org and no specific repos: can't proceed
    throw new Error(`Missing github.org in client config and no repos specified. ` +
...
export async function runReport(args) {
    ...
    if (org) {
        ...
        let hasMore = true;
        while (hasMore) {
            const search = await mcpSearchPRs(mcp, q, { perPage: 100, page, sort: 'updated', order: 'desc' });
        ...
        for (let batchIdx = 0; batchIdx < repoBatches.length; batchIdx++) {
            ...
            const reposConfig = { mode: 'allowlist', allowlist: batch };
